...
def train_lambda_rank(ARGS, data, model):
    ...
    for epoch in range(ARGS.epochs):
        ...
        for batch in range(ARGS.bpe):
            ...
            loss = lambda_rank_loss(scores, y)

            print((loss / y.shape[0]).sum())

            # loss_epoch.append(loss.item())

            # optimize
            optimizer.zero_grad()
            scores.backward(loss[:, None] / y.shape[0])
            optimizer.step()
...
def create_score_diffs(scores, gamma):
    return 1 / (1.0 + torch.exp(-gamma * (scores - scores.t())))
...
def rank_net_loss(scores, y, gamma):
    S = create_S(y)
    Sc = create_score_diffs(scores, gamma)
...
def irm_delta(irm_loss):
    pass


def lambda_rank_loss(scores, y, gamma=1, irm_type="ndcg"):
    # return rank_net_loss(scores, y) * irm_delta(irm_type)
    return rank_net_loss(scores, y, gamma)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n-hidden', type=int, default=100, help='number of hidden layer')
